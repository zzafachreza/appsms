...
export default function DataSanksi({navigation, route}) {
  ...
  const getDataList = () => {
    // setRefreshing(true);
    axios.get('https://zavalabs.com/sininja/api/sanksi.php').then(res => {
      setData(res.data);
  ...
  const renderItem = ({item}) => (
    <TouchableOpacity
      onPress={() => navigation.navigate('DataSanksiDetail', item)}
      style={{
        padding: 10,
        marginVertical: 10,
        borderBottomWidth: 3,
