...
export default function DataTindak({navigation, route}) {
  ...
  const getDataList = () => {
    // setRefreshing(true);
    axios
      .get('https://zavalabs.com/sininja/api/tindak_lanjut.php')
      .then(res => {
        setData(res.data);
  ...
  const renderItem = ({item}) => (
    <TouchableOpacity
      onPress={() => navigation.navigate('DataTindakInput', item)}
      style={{
        padding: 10,
        marginVertical: 10,
        borderBottomWidth: 3,
