...
export default function LaporanAduan({route}) {
    ...
    try {
      ...
      if (granted === PermissionsAndroid.RESULTS.GRANTED) {
        axios.post(myUrl2).then(res => {
          ...
          createPDF('Laporan_aduan', res.data);
        });
      } else {
  ...
  const createPDF = async (nama_file, html) => {
    let options = {
      html: html,
      fileName: 'Laporan_aduan' + nama_file,
      directory: '',
    };
  ...
  const myUrl = `https://zavalabs.com/sininja/api/aduan.php?tahun=${tahun}&bulan=${bulan}`;

  const myUrl2 = `https://zavalabs.com/sininja/api/aduan2.php?tahun=${tahun}&bulan=${bulan}`;
